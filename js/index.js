document.addEventListener('DOMContentLoaded', () => {
    // Data dummy untuk konten (sama seperti sebelumnya)
    const rilisData = [
    ...
    // Inisialisasi data dengan tanggal, views, dan slug
    ...
    function createCardHTML(item, index, animationClass = 'fade-in-bottom') {
        ...
        // Arahkan ke contoh halaman detail
        const detailUrl = item.id === 1 ? `rilis/${item.slug}.html` : '#'; 
        const linkTarget = item.id === 1 ? '' : 'alert("Halaman detail untuk rilis ini belum dibuat.")';

        return `<a href="${detailUrl}" ${item.id !== 1 ? `onclick='${linkTarget}; return false;'` : ''} class="release-card card-zoom bg-white rounded-lg shadow-lg overflow-hidden flex flex-col group w-full border border-gray-200 hover:shadow-xl transition-shadow duration-300 animate-on-scroll ${animationClass}" data-id="${item.slug}" data-delay="${index * 150}">
    ...
    // Render konten awal untuk halaman utama
    function renderInitialContent() {
        ...
        if (popularContainer) {
            // Duplikasi konten untuk efek marquee yang mulus
            const popularHTML = sortedByViews.slice(0, 6).map(item => createCardHTML(item, 0, '')).join('');
            popularContainer.innerHTML = popularHTML + popularHTML;
        }
        ...
        if (terbaruContainer) {
            terbaruContainer.innerHTML = sortedByDate.slice(0, 6).map((item, index) => createCardHTML(item, index, 'fade-in-bottom')).join('');
        }
    }
